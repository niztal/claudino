...
// One shared dir for the hooks, the status line, and the game. Deliberately
// NOT os.tmpdir(): TMPDIR differs between Claude Code's sandboxed shell, its
// hooks, and a freshly opened terminal window, and every claudino process must
// resolve the same path or they can't see each other's state.
const DIR =
  process.platform === 'win32' || !process.getuid
    ? path.join(os.tmpdir(), 'claudino')
    : path.join('/tmp', 'claudino-' + process.getuid());
